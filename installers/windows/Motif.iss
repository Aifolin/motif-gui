; Motif Boron Butterfly GUI Wallet Installer for Windows
; Copyright (c) 2017-2019, The Motif Project
; See LICENSE

[Setup]
AppName=Motif GUI Wallet
; For InnoSetup this is the property that uniquely identifies the application as such
; Thus it's important to keep this stable over releases
; With a different "AppName" InnoSetup would treat a mere update as a completely new application and thus mess up

AppVersion=0.14.1.2
DefaultDirName={pf}\Motif GUI Wallet
DefaultGroupName=Motif GUI Wallet
UninstallDisplayIcon={app}\motif-wallet-gui.exe
...
LicenseFile=LICENSE
AppPublisher=The Motif Developer Community
AppPublisherURL=https://getmotif.org
TimeStampsInUTC=yes
...
; Background info:
; This installer indeed asks for admin rights so the Motif files can be copied to a place where they have at least
...
; As of version 0.14.1.0, the Motif .exe files do not carry version info anyway in their .exe headers.
...
; applications somehow. Luckily this is no issue here because ALL files are "private" to Motif.

Source: "ReadMe.htm"; DestDir: "{app}"; Flags: ignoreversion
Source: "FinishImage.bmp"; Flags: dontcopy

; Motif GUI wallet exe and guide
Source: "bin\motif-wallet-gui.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\motif-gui-wallet-guide.pdf"; DestDir: "{app}"; Flags: ignoreversion

; Motif CLI wallet
Source: "bin\motif-wallet-cli.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\motif-gen-trusted-multisig.exe"; DestDir: "{app}"; Flags: ignoreversion

; Motif wallet RPC interface implementation
Source: "bin\motif-wallet-rpc.exe"; DestDir: "{app}"; Flags: ignoreversion

; Motif daemon
Source: "bin\motifd.exe"; DestDir: "{app}"; Flags: ignoreversion

; Motif daemon wrapped in a batch file that stops before the text window closes, to see any error messages
Source: "motif-daemon.bat"; DestDir: "{app}"; Flags: ignoreversion;

; Motif blockchain utilities
Source: "bin\motif-blockchain-export.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\motif-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\motif-blockchain-mark-spent-outputs.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\motif-blockchain-usage.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\motif-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\motif-blockchain-ancestry.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\motif-blockchain-depth.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\motif-blockchain-prune-known-spent-data.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\motif-blockchain-prune.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\motif-blockchain-stats.exe"; DestDir: "{app}"; Flags: ignoreversion
...
; Filename: "{app}\motif-wallet-gui.exe"; Description: "Run GUI Wallet now"; Flags: postinstall nowait skipifsilent
...
begin
  ...
  blockChainDefaultDir := ExpandConstant('{commonappdata}\bitmotif');
  s := 'The default folder to store the Motif blockchain is ' + blockChainDefaultDir;
  ...
  RegQueryStringValue(HKEY_CURRENT_USER, 'Software\motif-project\motif-core', 'blockchainDataDir', blockChainDir);
  if blockChainDir = '' then begin
    blockChainDir := GetPreviousData('BlockChainDir', '');
  end;
  if blockChainDir = '' then begin
    // Unfortunately 'TInputDirWizardDirPage' does not allow empty field, so "propose" Motif default location
...
begin
  Result := BlockChainDir('') + '\bitmotif.log';
...
begin
  if CurStep = ssPostInstall then begin
    // Re-build "motif-daemon.bat" according to actual install and blockchain directory used
    SetArrayLength(s, 3);
    s[0] := 'REM Execute the Motif daemon and then stay with window open after it exits';
    s[1] := '"' + ExpandConstant('{app}\motifd.exe') + '" ' + DaemonFlags('');
    s[2] := 'PAUSE';
    SaveStringsToFile(ExpandConstant('{app}\motif-daemon.bat'), s, false); 
  end;
end;
...
[Icons]
; Icons in the "Motif GUI Wallet" program group
; Windows will almost always display icons in alphabetical order, per level, so specify the text accordingly
Name: "{group}\GUI Wallet"; Filename: "{app}\motif-wallet-gui.exe";
Name: "{group}\GUI Wallet Guide"; Filename: "{app}\motif-GUI-guide.pdf"; IconFilename: "{app}\motif-wallet-gui.exe"
...
Name: "{group}\Utilities\Motif Daemon"; Filename: "{app}\motifd.exe"; Parameters: {code:DaemonFlags}
...
Name: "{group}\Utilities\Textual (CLI) Wallet"; Filename: "{app}\motif-wallet-cli.exe"; WorkingDir: "{userdocs}\Motif\wallets"
...
Name: "{group}\Utilities\x (Check Default Wallet Folder)"; Filename: "{win}\Explorer.exe"; Parameters: """{userdocs}\Motif\wallets"""
Name: "{group}\Utilities\x (Check GUI Wallet Log)"; Filename: "Notepad"; Parameters: """{userappdata}\motif-wallet-gui\motif-wallet-gui.log"""
Name: "{group}\Utilities\x (Try Daemon, Exit Confirm)"; Filename: "{app}\motif-daemon.bat"
Name: "{group}\Utilities\x (Try GUI Wallet Low Graphics Mode)"; Filename: "{app}\start-low-graphics-mode.bat"
Name: "{group}\Utilities\x (Try Kill Daemon)"; Filename: "Taskkill.exe"; Parameters: "/IM motifd.exe /T /F"

; Desktop icons, optional with the help of the "Task" section
Name: "{commondesktop}\GUI Wallet"; Filename: "{app}\motif-wallet-gui.exe"; Tasks: desktopicon
...
Root: HKCU; Subkey: "Software\motif-project"; Flags: uninsdeletekeyifempty
Root: HKCU; Subkey: "Software\motif-project\motif-core"; Flags: uninsdeletekey
Root: HKCU; Subkey: "Software\motif-project\motif-core"; ValueType: string; ValueName: "blockchainDataDir"; ValueData: {code:BlockChainDirOrEmpty};

; Configure a custom URI scheme: Links starting with "motif:" will start the GUI wallet exe with the URI as command-line parameter
; Used to easily start payments; example URI: "motif://<address>?tx_amount=5.0"
Root: HKCR; Subkey: "motif"; ValueType: "string"; ValueData: "URL:Motif Payment Protocol"; Flags: uninsdeletekey
Root: HKCR; Subkey: "motif"; ValueType: "string"; ValueName: "URL Protocol"; ValueData: ""
Root: HKCR; Subkey: "motif\DefaultIcon"; ValueType: "string"; ValueData: "{app}\motif-wallet-gui.exe,0"
Root: HKCR; Subkey: "motif\shell\open\command"; ValueType: "string"; ValueData: """{app}\motif-wallet-gui.exe"" ""%1"""

; Configure a custom URI scheme: Links starting with "motifseed:" will start the GUI wallet exe with the URI as command-line parameter
; Used to easily hand over custom seed node info to the wallet, with an URI of the form "motifseed://a.b.c.d:port"
Root: HKCR; Subkey: "motifseed"; ValueType: "string"; ValueData: "URL:Motif Seed Node Protocol"; Flags: uninsdeletekey
Root: HKCR; Subkey: "motifseed"; ValueType: "string"; ValueName: "URL Protocol"; ValueData: ""
Root: HKCR; Subkey: "motifseed\DefaultIcon"; ValueType: "string"; ValueData: "{app}\motif-wallet-gui.exe,0"
Root: HKCR; Subkey: "motifseed\shell\open\command"; ValueType: "string"; ValueData: """{app}\motif-wallet-gui.exe"" ""%1"""
