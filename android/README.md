...
# Base environnement
        ...
        cd motif/utils/build_scripts
        docker build -f android32.Dockerfile -t motif-android .
        cd ..

# Build GUI

        cd android/docker
        docker build -t motif-gui-android .
        docker create -it --name motif-gui-android motif-gui-android bash

# Get the apk

        docker cp motif-gui-android:/opt/android/motif-gui/build/release/bin/bin/QtApp-debug.apk .
...
- Using ADB (Android debugger bridge) : 
  ...
  The only place where we are allowed to play is `/data/local/tmp`. So : 
        ...
        adb  push /opt/android/motif-gui/build/release/bin/bin/QtApp-debug.apk /data/local/tmp
