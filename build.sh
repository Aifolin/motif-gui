...
if [ "$platform" == "darwin" ]; then
    BIN_PATH=$BIN_PATH/motif-wallet-gui.app/Contents/MacOS/
elif [ "$platform" == "mingw64" ] || [ "$platform" == "mingw32" ]; then
    MOTIFD_EXEC=motifd.exe
fi
...
get_tag
popd
echo "var GUI_MOTIF_VERSION = \"$TAGNAME\"" >> version.js
...
$QMAKE ../motif-wallet-gui.pro "$CONFIG" || exit
$MAKE || exit 

# Copy motifd to bin folder
if [ "$platform" != "mingw32" ] && [ "$ANDROID" != true ]; then
cp ../$MOTIF_DIR/bin/$MOTIFD_EXEC $BIN_PATH
fi
