...
function walletPathExists(directory, filename, isIOS, walletManager) {
    ...
    if(isIOS)
        var path = motifAccountsDir + filename;
    else
...
function getApproximateBlockchainHeight(_date, _nettype){
    // time of motif birth 2014-04-18 10:49:53 (1397818193)
    var motifBirthTime = _nettype == "Mainnet" ? 1397818193 : _nettype == "Testnet" ? 1410295020 : 1518932025;
    ...
    var secondsPerBlock;
    // before motif's birth
    if (requestedTime < motifBirthTime){
        console.log("Calculated blockchain height: 0, requestedTime < motifBirthTime " );
        return 0;
    }
    // time between during v1
    if (requestedTime > motifBirthTime && requestedTime < forkTime){
        approxBlockchainHeight = Math.floor((requestedTime - motifBirthTime)/secondsPerBlockV1);
